...
function CreateFolderModal({
  ...
  isOpen: boolean
  onClose: () => void
  onSubmit: (name: string, channelType: ChannelType) => void
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-[#52525B] mb-1">Folder Name</label>
            <Input
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g., Meta Ads Q1 2025"
              autoFocus
            />
          </div>
          <div className="mb-6">
            <label className="block text-sm font-medium text-[#52525B] mb-1">Channel Type</label>
            <select
              value={channelType}
...
export default function FoldersPage() {
  ...
  const filteredFolders = folders.filter(folder =>
    ...
    CHANNEL_LABELS[folder.channel_type].toLowerCase().includes(searchQuery.toLowerCase())
  )

  const handleCreateFolder = (name: string, channelType: ChannelType) => {
    const newFolder: Folder & { campaignCount: number } = {
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-[#52525B]">
            Organize your campaigns by channel or project.
          </p>
        </div>
          ...
          <div className="max-w-sm mx-auto">
            ...
            <p className="text-[#52525B] mb-6">
              {searchQuery
                ...
                : 'Create your first folder to start organizing campaigns.'}
            </p>
            {!searchQuery && (
              <Button variant="primary" onClick={() => setIsModalOpen(true)}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
